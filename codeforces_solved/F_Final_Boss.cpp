...
int main()
{
    ...
    while(t--)
    {
        ...
        vector<s64> attacks(n);
        vector<s64> cooldowns(n);
        for (auto &e: attacks) cin >> e;
        for (auto &e: cooldowns) cin >> e;

        set<pair<s64,int>> attack_cooldown;
        for (int i = 0; i < n; ++i)        
            attack_cooldown.insert({ 1, i });
        s64 l_turn = 1;
        while(h>0)
        {
            auto [turn, i] = *attack_cooldown.begin();
            attack_cooldown.erase(attack_cooldown.begin());
            l_turn = turn;
            h -= attacks[i];
            attack_cooldown.insert({ turn + cooldowns[i],i });
        }
